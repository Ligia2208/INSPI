...
namespace App\Http\Livewire\Centrosreferencia\Analiticamicolo;
...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        $count = Analiticamicolo::where('estado','=','A')->where('usuarior_id','=',0)->whereIn('sedes_id',$sedes_users)->whereIn('crns_id',$crns_users)->where('resultado_id','=',0)->count();
        $analiticastoxico = Analiticamicolo::where('estado','=','A')->where('usuarior_id','=',0)->whereIn('sedes_id',$sedes_users)->whereIn('crns_id',$crns_users)->where('resultado_id','=',0)->orderBy('codigo_calidad', 'desc');

        if($this->searchm){
            $analiticastoxico = $analiticastoxico->where('codigo_muestra', 'LIKE', "%{$this->searchm}%");
            $count = $analiticastoxico->count();
        ...
        if($this->searchc){
            ...
            $analiticastoxico = $analiticastoxico->whereIn('preanalitica_id',$preanaliticas);
            $count = $analiticastoxico->count();
        ...
        if($this->csedes){
            $analiticastoxico = $analiticastoxico->where('sedes_id', '=', $this->csedes);
            $count = $analiticastoxico->count();
        ...
        if($this->claboratorios){
            $analiticastoxico = $analiticastoxico->where('sedes_id', '=', $this->csedes)->where('crns_id','=',$this->claboratorios);
            $count = $analiticastoxico->count();
            $eventos = Evento::where('estado','=','A')->where('crns_id','=',$this->claboratorios)->orderBy('id', 'asc')->get();
        }

        if($this->ceventos){
            $analiticastoxico = $analiticastoxico->where('sedes_id', '=', $this->csedes)->where('crns_id','=',$this->claboratorios)->where('evento_id','=',$this->ceventos);
            $count = $analiticastoxico->count();
        }
        ...
        if($this->fechainicio){
            if ($this->fechafin){
                if ($this->fechainicio <= $this->fechafin){
                    ...
                    if($this->controlf==1){
                        $analiticastoxico = $analiticastoxico->where('fecha_toma', '>=', $this->fechainicio)->where('fecha_toma','<=',$this->fechafin);
                        $count = $analiticastoxico->count();

                    }
                    if($this->controlf==2){
                        $analiticastoxico = $analiticastoxico->where('fecha_llegada_lab', '>=', $this->fechainicio)->where('fecha_llegada_lab','<=',$this->fechafin);
                        $count = $analiticastoxico->count();
                    }
                    if($this->controlf==3){
                        $analiticastoxico = $analiticastoxico->where('fecha_procesamiento', '>=', $this->fechainicio)->where('fecha_procesamiento','<=',$this->fechafin);
                        $count = $analiticastoxico->count();
                    }
                }
                else{
        ...
        $analiticastoxico = $analiticastoxico->paginate($this->perPage);
        $this->emit('renderJs');

        return view('livewire.centrosreferencia.analiticatoxico.index', compact('count', 'analiticastoxico','sedes','crns','eventos'));
    }

    public function destroy($id)
    {
        try{
            $Analiticas = Analiticamicolo::findOrFail($id);
            $Analiticas->estado = 'I';
            $Analiticas->update();
    ...
    public function duplicate($id)
    {
        ...
        try{
            ...
            $newAnalitica = new Analiticamicolo();
