...
class Form extends Component
{
    ...
    public function mount(Preanaliticagen $Preanaliticastoxico, $method){
    ...
    public function render(){
        ...
        $this->emit('renderJs');
        return view('livewire.centrosreferencia.preanaliticagen.form', compact('sedes','sexos','pacientes','muestras','instituciones','estados','nacionalidades',
            'cantonprov','clases', 'usuarios'));
    }

    public function updatedselectedSede($sede_id){
        $config = SedeCrn::where('estado','=','A')->where('crns_id','=',9)->where('sedes_id','=',$sede_id)->orderBy('id', 'asc')->pluck('crns_id')->toArray();
    ...
    public function guardart(Preanaliticagen $pa, $ev){
    ...
    public function guardardetallet(Preanaliticagen $pa, $pre, $codigo, $ev){
        $absede = Sede::findOrFail($pa->sedes_id);
        $abcrn = Crn::findOrFail($pa->crns_id);
        $user = auth()->user()->id;
        $fecha_anio = date("Y");
    ...
    public function guardarp(Preanaliticagen $pa, $ev){
    ...
    public function guardarpigual(Preanaliticagen $pa, $ev, $codm){
    ...
    public function guardarmicolo(Preanaliticagen $pa, $ev){
        ...
        if($pa->primera_id>0){
            ...
            $newMuestra->codigo_muestra      = $codigo;
            $newMuestra->edad                = $pa->edad;
            $newMuestra->sexo                = $pa->paciente_sexo;
            if($tipogenera==1){
                $newMuestra->codigo_secuencial = 1;
            }
            else{
