...
namespace App\Http\Livewire\Soporte\Ticketusercerrado;
...
class Index extends Component
{
    ...
    {
        $user = Auth::user()->id;
        $count = Ticketsupport::where('status','=','A')->where('usuario_id','=',$user)->where('tecnico_id','>',1)->whereIn('estadoticket_id',[4,7])->count();
        $tickets = Ticketsupport::where('status','=','A')->where('usuario_id','=',$user)->where('tecnico_id','>',1)->whereIn('estadoticket_id',[4,7])->orderBy('id', 'DESC');
        ...
        return view('livewire.soporte.ticketusercerrado.index', compact('count', 'tickets'));
    }
