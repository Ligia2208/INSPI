...
class Index extends Component
{
    ...
    public function render()
    {
        ...
        if($existe>0){
            ...
            //dd($idTecnico); die();
            $count = Ticketsupport::where('status','=','A')->where('tecnico_id','=',$idTecnico[0]->id)->whereIn('estadoticket_id',[4,7])->count();
            $tickets = Ticketsupport::where('status','=','A')->where('tecnico_id','=',$idTecnico[0]->id)->whereIn('estadoticket_id',[4,7])->orderBy('prioridad_id', 'DESC')->orderBy('id', 'DESC');
        }
        else{
            $count = 0;
        ...
        return view('livewire.soporte.tickettecnicocerrado.index', compact('count', 'tickets'));
    }
