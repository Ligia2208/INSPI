...
namespace App\Http\Controllers\CentrosReferencia\Resultadomspindividual;
...
class ResultadomspindividualController extends Controller
{
    ...
    public function index(){
        return view('centrosreferencia.resultadomspindividual.index');
    }

    public function create(){
        $resultado = new Resultado();
        return view('centrosreferencia.resultadomspindividual.create', compact('resultado'));
    }

    public function show($resultado){
        $objRes = DB::table('inspi_crns.busqueda_paciente')->select('codigo','identidad','nombres','apellidos','fecha_recepcion','unidad_salud','sede','crn','evento','fecha_atencion','ingresa_por','usuario_registro','clase_muestra','tipo_muestra','fecha_toma_muestra','validado','fecha_validacion','usuarior_id')->where('codigo','=',$resultado)->orderby('codigo','asc')->get();
        $resultado = $objRes[0];

        $total = Analitica::where('preanalitica_id','=',$resultado->codigo)->where('estado','=','A')->count();

        if($resultado->validado=='S'){
            $responsable = User::findOrFail($resultado->usuarior_id);
            $nomres = $responsable->name;
        }
        else{
            $nomres = '';
        }

        $procesadas = Analitica::where('preanalitica_id','=',$resultado->codigo)->where('estado','=','A')->where('tecnica_id','>',0)->where('resultado_id','>',0)->where('usuarior_id','>',0)->count();
        if($total==$procesadas){
            $procesadot = 1;
            $analiticas = Analitica::where('preanalitica_id','=',$resultado->codigo)->where('estado','=','A')->where('tecnica_id','>',0)->where('resultado_id','>',0)->where('usuarior_id','>',0)->first();
            $tecnico = User::findOrFail($analiticas->usuarior_id);
            $nomtec = $tecnico->name;
        }
        else{
            $procesadot = 0;
        ...
        return view('centrosreferencia.resultadomspindividual.show', compact('resultado','procesadot','analiticas','nomtec','nomres'));
    }

    public function edit(Resultado $resultado){
        return view('centrosreferencia.resultadomspindividual.edit', compact('resultado'));
    }
    ...
    public function informemspanterior($id){
