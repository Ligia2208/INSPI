                    ...
                    <div class="card-body">
                        <div class="p-4 border rounded">
                            ...
                            @csrf
                                    ...
                                    <input type="hidden" id="id_area" name="id_area" value="{{ $datosLabora->id_area ?? '' }}" >
                                    <input type="hidden" id="id_zonal" name="id_zonal" value="{{ $datosLabora->czonal_id ?? '' }}" >

                                    <div id="contenedorEncuesta" class="row">

                                        <div class="col-md-12">
                                            <label for="descripLab" class="form-label">Descripción</label>
                                            <input type="text" id="descripLab" name="descripLab" class="form-control @error('descripLab') is-invalid @enderror" value="{{ old('descripLab', $datosLabora->descripcion ?? '') }}" required="" autofocus="" placeholder="Ingrese la descripción del laboratorio">
                                        ...
                                        <div class="col-md-6 mt-4">
                                            ...
                                            <select id="coordina" name="coordina" class="form-select single-select" required="" autofocus="">
                                                <option value="0">Seleccione la Coordinación</option>
                                                @foreach($coordina as $coor)
                                                    @if($coor->id == $datosLabora->czonal_id)
                                                    <option value="{{ $coor->id}}" selected> {{ $coor->nombre}} </option>
                                                    @else
                                        ...
                                        <div class="col-md-6 mt-2">
                                            ...
                                            @if(count($tipoEncuesta) > 0)
                                                @foreach($tipoEncuesta as $tipo)
                                                    @php
                                                        // Buscar si el tipo de encuesta actual está en los datos del laboratorio
                                                        $checked = $datosLaboraEncue->where('tipoencuesta_id', $tipo->id)->where('valor', 'true')->isNotEmpty();
                                                    @endphp

                                                    <div class="form-check form-switch">
                                                        <input class="form-check-input" type="checkbox" name="tipoEncuestaLab" id="flexSwitchPresencial{{ $tipo->id }}" data-id="{{ $tipo->id }}" {{ $checked ? 'checked' : '' }}>
                                                        <label class="form-check-label" for="flexSwitchPresencial{{ $tipo->id }}">{{ $tipo->nombre }}</label>
                                                    </div>
                                                @endforeach
                                            @else
