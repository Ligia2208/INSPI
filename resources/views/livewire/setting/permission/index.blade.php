        ...
        <div class="card card-custom gutter-b">
            ...
            <div class="card-header border-0 py-5">
                <h3 class="card-title align-items-start flex-column">
                    ...
                    <span class="text-muted mt-3 font-weight-bold font-size-sm">({{ $permissions->total() }})
                        Permisos(s)</span>
                </h3>
                <div class="card-toolbar">
                    <a href="#" wire:click.prevent="$emit('createFormPermission')"
                        ...
                        <tbody>
                            @forelse ($permissions as $permission)
                                <tr>
                                    <td>
                                        <span
                                            class="text-dark-75 font-weight-bolder d-block font-size-lg">{{ $permission->name }}</span>
                                    </td>
                                    <td>
                                        @forelse ($permission->roles as $role)
                                            <span
                                                class="font-weight-bolder font-size-lg badge badge-success">{{ $role->name }}</span>
                                        @empty
                                                ...
                                                <div class="symbol-group symbol-hover">
                                                    @foreach ($permission->users as $user)
                                                        <div class="symbol symbol-30 symbol-circle"
                                                            ...
                                                            <img alt="{{ $user->name }}"
                                                                @if ($user->image) src="{{ Storage::url($user->image->url) }}"
                                                                @else
                                                                    src="{{ asset('assets/media/users/blank.png') }}" @endif>
                                                        </div>
                                                        @if ($loop->index == 8)
                                                            <div class="symbol symbol-30 symbol-circle symbol-light">
                                                                <span
                                                                    class="symbol-label font-weight-bold">{{ $role->users->count() - ($loop->index + 1) }}+</span>
                                                            </div>
                                                        @break
                                                    @endif
                                                @endforeach
                                ...
                                <td align="center">
                                    <i class="navi-item"
                                        wire:click.prevent="$emit('editFormPermission', {{ $permission->id }})">
        ...
        <div class="card-body">
            <div class="card-px text-center py-5">
                ...
                <p class="text-gray-400 fs-4 fw-bold mb-10">Al parecer no tienes ningun permiso.
                    <br> Ponga en marcha su CRM añadiendo su primer permiso
                </p>
                <a wire:click.prevent="$emit('createFormPermission')" href="#" class="btn btn-primary">Agregar
                    permiso</a>
            </div>
            <div class="text-center px-4 ">
                <img class="img-fluid col-6" alt=""
                    src="{{ asset('assets/media/ilustrations/work.png') }}">
            </div>
        </div>
    </div>
@endif
