                                ...
                                <div class="d-flex justify-content-end">
                                    <div class="dropdown dropdown-inline" data-toggle="tooltip"  data-placement="left">
                                        ...
                                        <div class="dropdown-menu dropdown-menu-sm dropdown-menu-right">
                                            <a class="dropdown-item" href="{{ route('seguimiento.show', $seguimiento) }}"><i class="fa fa-eye mr-2"></i> Ver</a>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="card-body">
                            ...
                            <div class="d-flex flex-column align-items-center">
                                ...
                                <div>
                                    ...
                                    {{ $seguimiento->fechaasignaciondg }}
                                </div>
                                <a href="{{ route('seguimiento.show', $seguimiento) }}">
                                    <div>
                                        <img alt="{{ $seguimiento->numerodocumento }}" class="max-h-35px" src="{{ asset('assets') }}/media/svg/files/pdf.svg"/>
                                        <!--begin: Tite-->
                                        <p></p>
                                        <p>{{ $seguimiento->numerodocumento }} ({{ $seguimiento->solicitud->fechadocumento }})</p>                                        
                                    </a>
                                <p class="text-dark-75 font-weight-bold my-4 font-size-lg">{{ $seguimiento->area->nombre }}</p>
                                </div> 
                                </center>
                                @if ($seguimiento->estado_id==1)
                                <span class="btn btn-light-warning btn-sm font-weight-bold btn-text">{{ $seguimiento->estadosolicitud->nombre }}</span>
                                @endif
                                @if ($seguimiento->estado_id==2)
                                <span class="btn btn-light-primary btn-sm font-weight-bold btn-text">{{ $seguimiento->estadosolicitud->nombre }}</span>
                                @endif
                                @if ($seguimiento->estado_id==3)
                                <span class="btn btn-light-info btn-sm font-weight-bold btn-text">{{ $seguimiento->estadosolicitud->nombre }}</span>
                                @endif
                                @if ($seguimiento->estado_id==4)
                                <span class="btn btn-light-danger btn-sm font-weight-bold btn-text">{{ $seguimiento->estadosolicitud->nombre }}</span>
                                @endif
                                @if ($seguimiento->estado_id==5)
                                <span class="btn btn-light-success btn-sm font-weight-bold btn-text">{{ $seguimiento->estadosolicitud->nombre }}</span>
                                @endif
