                ...
                <div class="row">
                    <div class="col-lg-12 margin-tb">
                        <div class="pull-right mb-2">
                            <a id="createRequerimiento" class="btn btn-primary btn-shadow font-weight-bold mr-2" href="javascript:void(0)">
                ...
                <div class="card card-custom">
                    <div class="card-body">
                        <div class="table-responsive">
                            <table class="table table-head-custom table-head-bg table-borderless table-vertical-center" id="datatable-requerimiento">
                                <thead>
                                    <tr>
                                        <th>Id</th>
                                        <th>Id_Area</th>
            ...
            <script type="text/javascript">
                $(document).ready( function () {
                    ...
                    $('#datatable-requerimiento').DataTable({
                        processing: true,
                        serverSide: true,
                        ajax: "{{ url('requerimiento') }}",
                        columns: [
                            {   data: 'id',                 name: 'id', 'visible': false},
                            {   data: 'id_area',            name: 'id_area', 'visible': false},
                            ...
                            {   data: null,
                                render: function (data, type, full, meta) {
                                    ...
                                    array = '<a title="Editar Requerimiento" id="editRequerimiento" href="javascript:void(0)" data-id="'+ full.id +'" data-fecha_solicitud="'+ full.fecha_solicitud +'" data-numero_pedido="'+ full.numero_pedido +'" data-area="'+ full.area +'" data-id_area="'+ full.id_area +'" data-hora_solicitud="'+ full.hora_solicitud +'" data-solicitante="'+ full.solicitante +'" data-material_lye="'+ full.material_lye +'" data-cant_completa="'+ full.cant_completa +'" data-auxiliar_mdc="'+ full.auxiliar_mdc +'" data-responsable_mdc="'+ full.responsable_mdc +'" data-estado="'+ full.estado +'" data-toggle="tooltip" data-original-title="Edit" class="edit">'
                                        + '<i class="ace-icon fa fa-edit" style="color:green"></i>'
                                        + '</a>'
                                        + ' '
                                        + '<a title="Eliminar Requerimiento" id="deleteRequerimiento" href="javascript:void(0)" data-id="'+ full.id +'" data-toggle="tooltip" data-original-title="Delete" class=" delete">'
                    ...
                    $(document).on('click', '#deleteRequerimiento', function () {
                        if (confirm("Desea eliminar registro?") == true) {
                            var id = $(this).data('id');
                            // ajax
                            $.ajax({
                                    type:"POST",
                                    url: "{{ url('requerimiento.delete') }}",
                                    data: { id: id,
                                            _token: "{{ csrf_token() }}"
                                    },
                                    dataType: 'json',
                                    success: function(res){
                                    var oTable = $('#datatable-requerimiento').dataTable();
                                    oTable.fnDraw(false);
                                    }//fin success
                            });//fin ajax
                        }//fin if
                ...
                /*function RefrescaProducto(){
                    var ip = [];
                    var i = 0;
                    $('#btnGuardarProducto').addClass('hidden'); //Deshabilito el Boton Guardar
                    ...
                    if (i > 0) {
                        $('#btnGuardarProducto').removeClass('hidden');
                    }
