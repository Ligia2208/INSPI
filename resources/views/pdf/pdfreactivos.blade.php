...
<body>
    ...
    <div class="container mt-10">
        <table>
            <tr>
                <td colspan="11" class="text-left"> <strong> CIUDAD / ZONAL: </strong> <span> {{$results->nombre_zonal}} </span> </td>
            </tr>
            <tr>
                <td colspan="11" class="text-left"> <strong> CENTRO NACIONAL DE REFERENCIA / PLATAFORMA / ÁREA: </strong> <span> {{$results->nombre_laboratorio}} </span> </td>
            </tr>
            <tr>
                <td colspan="11" class="text-left"> <strong> NOMBRE DEL REACTIVO / FUNGIBLE: </strong> <span> @if(!empty($datosArt->descripcion))
            ...
            <tr>
                <th>SALDO INICIAL</th>
                <th>CANTIDAD INICIAL DE REACTIVO/FUNGIBLE</th>
                <th>FECHA DE LA SEMANA DE EGRESO</th>
                <th>NOMBRE DE PRUEBA</th>
                <th>CANTIDAD DE REACTIVO / FUNGIBLE POR PRUEBA</th>
                <th>NUMERO DE PRUEBAS POR SEMANA</th>
                <th>EGRESO/INGRESO</th>
                <th>SALDO DE REACTIVO / FUNGIBLE</th>
                <th>STOCK MÍNIMO (SOLICITAR REACTIVO / FUNGIBLE)</th>
                <th>RESPONSABLE</th>
                <th>OBSERVACIONES</th>
            </tr>
            ...
            @if($movimi->tipo_movimiento == 'Ingreso' || $movimi->tipo_movimiento == 'Compra local' || $movimi->tipo_movimiento == 'Donación')
            <tr>

                <?php 

                $cantidadFinal += $movimi->cantidad;
                //$cantidadInicial += $movimi->ini_saldo;

                ?>

                <td>{{$cantidadInicial}}</td>
                <td>{{$movimi->ini_saldo}}</td> 
                <td>{{$movimi->fecha}}</td>
                <td>Ingreso de reactivo</td>
                <td></td>
                <td></td>
                <td>{{$movimi->cantidad}}</td>
                <td>{{$cantidadFinal}}</td>
                <td></td>
                <td>{{$movimi->usuario}}</td>
                <td>Ingresan reactivos con fecha {{$movimi->fecha}}</td>
            ...
            @elseif($movimi->tipo_movimiento == 'Transferencia')
            <tr>
                ...
                <td> 0 </td> 
                <td>{{$movimi->fecha}}</td>
                <td>Transferencia de reactivo</td>
                <td></td>
                <td></td>
                <td>{{$movimi->cantidad}}</td>
                <td>{{$cantidadFinal}}</td>
                <td></td>
                <td>{{$movimi->usuario}}</td>
                <td>Transferencia de reactivos con fecha {{$movimi->fecha}}</td>
            ...
            @elseif($movimi->tipo_movimiento == 'Ajuste')
            <tr>
