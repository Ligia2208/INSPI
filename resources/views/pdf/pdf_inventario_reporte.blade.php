...
<body>
        ...
        <div style="margin-top: 0px; position: absolute; top: 0; left: 0; height: 50px; width: 100%;" >
            ...
            <div style="width: 65%; height: 50px; position: absolute; top: 0; left: 15%;">
                ...
                <div class="borde" style="width: 100%; height: 30px; position: absolute; top: 0; left: 0; border: 1px solid black; text-align: center; line-height: 21px;">
                    <strong style="font-size: 12px;"> REPORTE GENERAL </strong>
                </div>
    ...
    <div class="container mt-10">
        <table>
            <tr>
                <td colspan="8" class="text-left"> <strong> CIUDAD / ZONAL: </strong> <span> PLANTA CENTRAL </span> </td>
            </tr>
            <tr>
                <td colspan="8" class="text-left"> <strong> CENTRO NACIONAL DE REFERENCIA / PLATAFORMA / ÁREA: </strong> <span> INSPI </span> </td>
            </tr>
            <tr>
                <td colspan="8" class="text-left"> <strong> FECHA: </strong> <span> {{$fecha}} </span> </td>
            </tr>
            <tr>
                <td colspan="8" class="text-left"> <strong> FILTRADO POR: </strong> <span> {{$filtro1}} - {{$filtro2}} </span> </td>
            </tr>
            ...
            <tr>
                ...
                <th> STOCK </th>
                <th> F. CADUCA </th>
                <th> DIAS PARA CADUCAR </th>
            </tr>

            @foreach($articulos as $reporte)
            <tr>
                <td>{{ $reporte->id }}</td>
                <td>{{ $reporte->nombre }}</td>
                <td>{{ $reporte->lote }}</td>
                <td>{{ $reporte->categoria }}</td>
                <td>{{ $reporte->unidad }}({{ $reporte->categoria }})</td>
                <td>{{ $reporte->stock }}</td>
                <td>{{ $reporte->f_caduca }}</td>
                <td>{{ $reporte->dias_para_caducar }}</td> 
            </tr>
            @endforeach
