...
<head>
    <meta charset="utf-8">
    <title>Registro de Stock en Bodega</title>
    <style>
        body {
...
<body>
        ...
        <div style="margin-top: 0px; position: absolute; top: 0; left: 0; height: 50px; width: 100%;" >
            ...
            <div style="width: 65%; height: 50px; position: absolute; top: 0; left: 15%;">
                ...
                <div class="borde" style="width: 100%; height: 30px; position: absolute; top: 0; left: 0; border: 1px solid black; text-align: center; line-height: 21px;">
                    <strong style="font-size: 12px;">REGISTRO DE STOCK EN BODEGA - REPORTE GENERAL</strong>
                </div>
    ...
    <div class="container mt-10">
        <table>
            <tr>
                <td colspan="7" class="text-left"> <strong> CIUDAD / ZONAL: </strong> <span> {{$results->nombre_zonal}} </span> </td>
            </tr>
            <tr>
                <td colspan="7" class="text-left"> <strong> CENTRO NACIONAL DE REFERENCIA / PLATAFORMA / ÁREA: </strong> <span> {{$results->nombre_laboratorio}} </span> </td>
            </tr>
            <tr>
                <td colspan="7" class="text-left"> <strong> FECHA: </strong> <span> {{$fecha}} </span> </td>
            </tr>
            ...
            <tr>
                ...
                <td>{{ $reporte->art_nombre }}</td>
                <td>{{ $reporte->uni_nombre }}({{ $reporte->uni_abreviatura }})</td>
                <td>{{ $reporte->lot_nombre }}</td>
                <td>{{ $reporte->f_caduca }}</td>
                <td>{{ $reporte->cantidad }}</td>
                <td>{{ $reporte->cant_minima }}</td>
            </tr>
            @endforeach
